 ...
 * @param size - Optional specific unit to format to
...
// Context type excludes root and input props (handled internally)
 ...
 * Wraps children with drag-and-drop functionality and state management
 *
 * Features:
 * - Visual feedback for drag states (active, success, error)
 * - Automatic border styling based on state
...
/**
 * DropzoneContent component
 * Displays selected files with preview, status, and remove functionality
 * Shows upload button when files are ready
 */
      ...
      {files.map((file, idx) => {
        ...
        return (
          ...
          >
            ...
            {/* Remove button (hidden during upload or after success) */}
            {!loading && !isSuccessfullyUploaded && (
              <Button
                size="icon"
                variant="link"
...
/**
 * DropzoneEmptyState component
 * Shows initial state with upload instructions and limits
 */
const DropzoneEmptyState = ({ className }: { className?: string }) => {
  const { maxFiles, maxFileSize, inputRef, isSuccess } = useDropzoneContext();

  // Hide when upload is successful
  if (isSuccess) {
    return null;
  }
